...
def get_children(parent, bn):
    children = []
    for conditional in bn["structure"]:
        if '|' in conditional:
            [var, parents_of_conditional] = conditional[2:-1].split("|")
            parents_of_conditional = parents_of_conditional.split(",")
            if parent in parents_of_conditional:
                children.append(var)

    return children


def markov_blanket(V, bn):
    variables = [V]
    parents = get_parents(V, bn)
    if parents is not None:
        parents = parents.split(",")
        variables += parents
    children = get_children(V, bn)
    if children is not None:
        variables += children
        for child in children:
            p = get_parents(child, bn)
            if p is not None:
                for p in p.split(","):
...
def get_probability_given_markov_blanket(V, evidence, bn):
    mb = markov_blanket(V, bn)
    C = {}
    for domain_v in get_domain_values(V, bn):
        C[domain_v] = 0
        probability = 1
        local_evidence = evidence.copy()
        local_evidence[V] = domain_v
        for var in mb:
            probability *= get_probability_given_parents(var, local_evidence[var], local_evidence, bn)
        C[domain_v] = probability
